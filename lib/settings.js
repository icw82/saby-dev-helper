...
messages.binaries.notFound = (strings, path) => `
    ...
    ${ path }
`;
messages.binaries.unexpectedType= (strings, path) => `
    Називестный тип дистрибутива
    ${ path }
`;
...
const getSettings = () => {
    ...
    const distribType = genieMeta.Binaries[0].type;

    if (distribType === 'unpacked') {
        locals.binaries = resolve(genieMeta.Binaries[0].path);

    } else if (distribType === 'zip') {
        locals.binaries = getGenieUnpackedDistrib(locals.s3cld);

    } else {
        throw new Error(chalk.red(
            messages.binaries.unexpectedType`${ distribType }`
        ));
    }
